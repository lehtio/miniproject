...
test('Filtering sorts multiple articles correctly', async () => {
    ...
    await user.type(authorInput, 'Linna Vaino')
    ...
    await user.type(keyInput, 'testkey')
    await user.type(authorInput, 'Kunnas Mauri')
    await user.type(titleInput, 'test title')
    await user.type(journalInput, 'test journal')
    await user.type(yearInput, '2023')
    await user.type(volumeInput, 'test volume')
    ...
    await user.type(keyInput, 'testkey')
    await user.type(authorInput, 'Kivi Aleksi')
    await user.type(titleInput, 'test title')
    await user.type(journalInput, 'test journal')
    await user.type(yearInput, '2023')
    await user.type(volumeInput, 'test volume')
    ...
    setTimeout(() => {
        const ekalisays = screen.getByText('Linna Vaino.')
        const tokalisays = screen.getByText('Kunnas Mauri.')
